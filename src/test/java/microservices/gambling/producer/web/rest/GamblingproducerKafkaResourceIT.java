...
import microservices.gambling.producer.config.KafkaProperties;
...
class GamblingproducerKafkaResourceIT {
    ...
    void setup() {
        ...
        GamblingproducerKafkaResource kafkaResource = new GamblingproducerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/gamblingproducer-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/gamblingproducer-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
